...
class MeterReadingDataset(torch.utils.data.Dataset):
    def __init__(self, root, transforms):
        ...
        self.imgs = list(sorted(os.listdir(os.path.join(root, "digitalMeterImages"))))  # this will be the orginal image
        self.masks = list(sorted(os.listdir(os.path.join(root, "digital_annotate")))) # this will be the annotated image
        self.values = pd.read_csv(os.path.join(root, "annotate.csv"), converters={'value': str})

    def __getitem__(self, idx):
        # load images and masks
        #print("let me see:", self.imgs[idx], idx)
        img_path = os.path.join(self.root, "digitalMeterImages", self.imgs[idx])
        mask_path = os.path.join(self.root, "digital_annotate", self.masks[idx])
...
def main():
    ...
    torch.save(model, "digitalModel.pt")
    print("saved")
