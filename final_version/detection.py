...
class MeterReadingDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        id = int(self.masks[idx].split('.')[0])
        
        labels = torch.zeros((1,), dtype=torch.int64)
        if id < 124:
            labels[0] = 1
        elif id > 124 and id < 194:
            labels[0] = 2
        else:
            labels[0] = 3
...
def load_model():
    num_classes = 4 # meter reading and background
...
def get_dataloader():
    ...
    dataset = MeterReadingDataset('../data/trainData', get_transform(train=True))
    dataset_test = MeterReadingDataset('../data/trainData', get_transform(train=False))
...
def main():
    ...
    torch.save(model, "detectionModel.pt")
    print("saved")
